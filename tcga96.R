...
data <- read.csv("data/counts.TCGA.WES.SBS-96.tsv", sep='\t', row.names=1, header=T)
vocab <- colnames(data)
...
if (!dir.exists('output/tcga96')){
  dir.create('output/tcga96')
}
...
for (i in 0:(cv.iters-1)){
  for (K in Ks){
    SAGE.betas.fp = sprintf('output/tcga96/sage-K%d-iter%d-betas.tsv', K, i)
    LDA.betas.fp = sprintf('output/tcga96/ctm-K%d-iter%d-betas.tsv', K, i)
...
write.table(df, file='output/tcga96/perplexities.tsv', quote=F, sep='\t',
